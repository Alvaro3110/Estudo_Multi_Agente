SELECT 
    ...
    SUM(st.quantity * (p.price - p.cost)) AS total_profit,
    SUM(st.quantity * p.cost) AS total_cost,
    (SUM(st.total_amount) - SUM(st.quantity * p.cost)) / NULLIF(SUM(st.total_amount), 0) AS profit_margin
FROM 
